...
- Add `streamableHttpClientChannel`, posting each client message as a
  ...
  answered with a body or a JSON reply whose id does not match the request's.
  A `tools/call` whose params or arguments are not a string-keyed map is a
  request error. Valid `x-mcp-header` annotations from `tools/list` are
  mirrored on later `tools/call` requests, including an integer written as a
  decimal, and a value the tool cannot carry fails the request instead of
  going out without its header. Invalid tool definitions are dropped, and a
  `tools/list` page with no cursor replaces what earlier pages taught. The
  helper speaks only 2026-07-28 and does not negotiate a version.
