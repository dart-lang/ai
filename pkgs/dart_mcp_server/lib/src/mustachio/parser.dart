...
/// Fork of https://github.com/dart-lang/dartdoc/blob/main/lib/src/mustachio/parser.dart
library;
...
class MustachioParser {
  ...
  List<MustachioNode> _parseBlock({String? sectionKey}) {
    ...
    void trimTextRight() {
      ...
      while (true) {
        ...
        final ch = template.codeUnitAt(newEndIndex - 1);
        if (ch == $space || ch == $tab) {
          newEndIndex--;
        } else if (ch == $cr || ch == $lf) {
  ...
  _TagParseResult _parseTag() {
    final tagStartIndex = _index - 2;
    _walkPastWhitespace();
    if (_atEnd) {
      return _TagParseResult.endOfFile;
    }
    final char = _thisChar;
    if (char == $hash) {
      _index++;
      _walkPastWhitespace();
  ...
  _TagParseResult _parsePartial({required int tagStartIndex}) {
    ...
    final key = template.substring(startIndex, endIndex);
    final keySpan = _sourceFile.span(startIndex, endIndex);
  ...
  _TagParseResult _parseSection({
    ...
    final children = _parseBlock(sectionKey: parsedKey.joinedNames);
    final span = _sourceFile.span(tagStartIndex, _index);
    final parsedKeySpan = parsedKey.span!;
    ...
    if (parsedKey.names.length > 1) {
      ...
      final lastName = parsedKey.names.last;
      final keySpanEndOffset = parsedKeySpan.end.offset;
      final lastNameSpan = _sourceFile.span(
  ...
  _TagParseResult _parseVariable({required int tagStartIndex}) {
    ...
    final parsedKey = _parseKey(escape: escape);
    if (parsedKey.type == _KeyParseResultType.notKey) {
      return _TagParseResult.notTag;
    } else if (parsedKey == _KeyParseResult.endOfFile) {
      return _TagParseResult.endOfFile;
  ...
  _KeyParseResult _parseKey({bool escape = true}) {
    ...
    final key = template.substring(startIndex, _index);
    final span = _sourceFile.span(startIndex, _index);
    ...
    final char0 = _thisChar;
    final char1 = _nextChar;
    if (escape) {
