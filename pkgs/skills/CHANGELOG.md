...
- Widen the `cli_util` dependency constraint to allow version `0.6.x`.

## 1.0.1

- Expand the `SKILL.md` template generated by `skills create` to include
  structured instruction sections and authoring guidelines.
- Fix `skills list` to ignore uninstalled skills (`isInstalled: false`) and skills
